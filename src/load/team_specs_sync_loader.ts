...
export default class TeamSpecsSyncLoader extends AllSpecsSyncLoader {
    ...
    parse(input: SmartAPIQueryResult): SmartAPISpec[] {
        return input.hits.filter(item => 'x-translator' in item.info && "team" in item.info["x-translator"] && Array.isArray(item.info["x-translator"].team) && item.info["x-translator"].team.includes(this._teamName));
    }
}